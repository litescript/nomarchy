...
| **Webapps** | ~15 `.desktop` launchers on the old disk (ChatGPT, Figma, GitHub, Google Contacts/Maps/Messages/Photos, Proxmox, Twitch, WhatsApp, YouTube, Zoom). **Not wanted** — Pete does not use them (2026-09-10) and will install anything he misses. Treat as closed, not outstanding. Same for the old Signal and 1Password keys: 1Password is used as a browser plugin, not an app. |
| **launch-or-focus** | Rebuilt as `common/scripts/launch-or-focus`. Bound on `Mod+M/F/T/D/O` (Spotify, shelf, btop, lazydocker, Obsidian). `Mod+B` deliberately stays a plain launch — duplicating Firefox windows is wanted behaviour, confirmed by Pete. |
